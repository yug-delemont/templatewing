  ...
  $(document).on("submit", ".contact-form", function () {
    $.ajax({
      ...
      success: function (data) {
        $(".form-respond").html(
          "<div class='content-message success'><h2>Email Sent Successfully Your message has been submitted.</h2></div>"
        );
      },
      error: function (xhr, err) {
        $(".form-respond").html(
          "<div class='content-message'><h2>Error sending Try again later.</h2></div>"
        );
      },
    });
    return false;
...
function applyBloom(source, destination) {
  if (bloomFramebuffer.length < 2) return;
  ...
  for (let i = bloomFramebuffer.length - 2; i >= 0; i--) {
    let baseTex = bloomFramebuffer[i];
    gl.uniform2f(
      bloomBlurProgram.uniforms.texelSize,
      1.0 / last.width,
      1.0 / last.height
